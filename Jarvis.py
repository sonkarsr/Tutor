...
class Jarvis(CmdInterpreter, object):
    ...
    first_reaction_text += Fore.BLUE + \
        'Tutor\' sound is by default disabled.' + Fore.RESET
    first_reaction_text += "\n"
    first_reaction_text += Fore.BLUE + 'In order to let Tutor talk out loud type: '
    ...
    def default(self, data):
        ...
        print_say("I could not identify your command Sarvesh...", self, Fore.WHITE)
    ...
    def postcmd(self, stop, line):
        ...
        if self.first_reaction:
            self.prompt = (
                Fore.RED +
                "{} Tell me something else Sarvesh?\n".format(PROMPT_CHAR) + Fore.RESET
            )
            self.first_reaction = True
        if self.enable_voice:
            self.speech.text_to_speech("Tell me something else  Sarvesh?\n")
